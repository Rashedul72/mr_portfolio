...
export default function About() {
  ...
  const cardBase =
    "glass rounded-2xl hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-300";
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
          ...
          >
            ...
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p className="border-l-2 border-cyan-500/40 pl-4">
                ...
                specialize in creating innovative solutions that bridge
                technology and real-world applications.
              </p>
              <p className="border-l-2 border-violet-500/40 pl-4">
                With experience spanning e-commerce platforms, agricultural IoT
                systems and many more, I bring a diverse skill set to every
                project. I&apos;m currently leading development at Better Emart
                while working on cross-platform mobile solutions.
              </p>
              <p className="border-l-2 border-rose-500/40 pl-4">
                My passion lies in leveraging technology to solve complex
                problems, from optimizing agricultural processes with AI to
                creating seamless shopping experiences across multiple
                platforms.
              </p>
            </div>
          </motion.div>
          ...
          {/* <motion.div
            className={`col-span-2 lg:col-span-4 ${cardBase} p-6 sm:p-8 overflow-hidden relative`}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.8 }}
            viewport={{ once: true, amount: 0.15 }}
          >
            <div className="pointer-events-none absolute -top-20 -right-20 w-56 h-56 bg-cyan-500/10 rounded-full blur-3xl" />
            <div className="pointer-events-none absolute -bottom-24 -left-20 w-56 h-56 bg-violet-500/10 rounded-full blur-3xl" />

            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
              <div>
                <h4 className="text-lg sm:text-xl font-semibold text-white">Core Competencies</h4>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  Strong foundations across product engineering and modern web delivery
                </p>
              </div>
              <span className="inline-flex items-center self-start sm:self-auto px-3 py-1 rounded-full border border-cyan-400/30 bg-cyan-500/10 text-cyan-300 text-[11px] sm:text-xs font-medium tracking-wide">
            ...
            <div className="relative z-10 grid sm:grid-cols-2 gap-4">
              {skills.map((skill, index) => (
                <motion.div
                  key={skill.name}
                  className="rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-4 sm:px-5 sm:py-5 hover:bg-white/[0.05] transition-colors duration-300"
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm sm:text-[15px] font-medium text-slate-200">{skill.name}</span>
                    <span className="text-[11px] sm:text-xs font-mono text-cyan-300 px-2 py-0.5 rounded-md border border-cyan-400/30 bg-cyan-500/10">
                      {skill.percentage}%
                    </span>
                  </div>
                  <div className="w-full bg-white/[0.06] rounded-full h-2 overflow-hidden">
                    <motion.div
                      className="relative bg-gradient-to-r from-cyan-400 via-sky-400 to-violet-500 h-2 rounded-full shadow-[0_0_10px_rgba(34,211,238,0.35)]"
                      initial={{ width: 0 }}
